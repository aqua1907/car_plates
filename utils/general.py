...
def scale_outputs(outputs, image, original_image):
    for output in outputs:
        if output is not None and len(output):
            output[:, :4] = scale_coords(image.shape[2:], output[:, :4], original_image.shape).round()

    return reversed(outputs)


def detect(original_image, model, device, half, size=640, iou_tresh=0.6, conf=0.1):
    img = letterbox(original_image, new_shape=size)[0]
    img = convert(img, device, half)
    preds = model(img, augment=False)[0]
    preds = non_max_suppression(preds, conf_thres=conf, iou_thres=iou_tresh, classes=0, agnostic=False)
    preds = scale_outputs(preds, img, original_image)

    return preds


def get_characters(image, preds, classes, visualize=False):
    charList = []
    positions = []
    for character in preds:
        if character is not None:
            for *xyxy, conf, cls in character:
                x1, y1, x2, y2 = int(xyxy[0]), int(xyxy[1]), int(xyxy[2]), int(xyxy[3])
                if visualize:
                    cv2.rectangle(image, (x1, y1), (x2, y2), (255, 0, 0), thickness=1, lineType=cv2.LINE_AA)
                charList.append(classes[int(cls)])
                positions.append(x1)
...
def cv2_show(image, targets, classes, colors):
    for *xyxy, conf, label in targets:
        x1, y1, x2, y2 = int(xyxy[0]), int(xyxy[1]), int(xyxy[2]), int(xyxy[3])
        idx = int(label)
        label = classes[idx]
        ...
        cv2.rectangle(image, (x1, y1), (x2, y2), colors[idx], 2, lineType=cv2.LINE_AA)

        x2, y2 = x1 + t_size[0], y1 - t_size[1] - 3
        cv2.rectangle(image, (x1, y1), (x2, y2), colors[idx], -1)
        cv2.putText(image, label, (x1, y1-2), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 0), 1, lineType=cv2.LINE_AA)
