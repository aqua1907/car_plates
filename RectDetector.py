...
class RectDetector(object):
    ...
    async def get_cv_zonesRGB_async(self, img, rects, gw=0, gh=0, coef=4.6, auto_width_height=True):
        ...
        promises = [loop.create_task(
            self.get_cv_zoneRGB_async(img, rect, gw=gw, gh=gh, coef=coef, auto_width_height=auto_width_height)) for rect
                    in rects]
        if bool(promises):
    ...
    def makeAligmentMatrix(self, line, arr, points):
        if len(arr) == 0:
            ...
            return self.linearLineMatrixCrossPoint(points[pObj["sPoint"]][0], points[pObj["ePoint"]][0],
                                                   points[pObj["cPoint"]][0])
    ...
    def addOffset(self, targetPoints, offsetHorisontal, offsetVertical):
        ...
        for i in range(cnt):
            ...
            points.append(
                self.detectIntersectionNormDD(distanсes[iPrev]['matrix'], distanсes[iNext]['matrix'], offset1, offset2))
        ...
        if d0 < d1:
            ...
            if abs(angle0) > abs(angle1):
                if distanсes[2]["d"] < distanсes[0]["d"]:
                    points[3] = self.detectIntersectionParalelLine(distanсes[2], distanсes[1],
                                                                   points[distanсes[0]["p0"]])
                else:
                    points[0] = self.detectIntersectionParalelLine(distanсes[0], distanсes[1],
                                                                   points[distanсes[2]["p1"]])
            else:
                if distanсes[2]["d"] < distanсes[0]["d"]:
                    points[2] = self.detectIntersectionParalelLine(distanсes[2], distanсes[3],
                                                                   points[distanсes[0]["p1"]])
                else:
                    points[1] = self.detectIntersectionParalelLine(distanсes[0], distanсes[3],
                                                                   points[distanсes[2]["p0"]])
        else:
            angle1 = distanсes[1]["coef"][3] - distanсes[2]["coef"][3]
            if abs(angle0) > abs(angle1):
                if distanсes[3]["d"] < distanсes[1]["d"]:
                    points[3] = self.detectIntersectionParalelLine(distanсes[3], distanсes[0],
                                                                   points[distanсes[1]["p1"]])
                else:
                    points[2] = self.detectIntersectionParalelLine(distanсes[1], distanсes[0],
                                                                   points[distanсes[3]["p0"]])
            else:
                if distanсes[3]["d"] < distanсes[1]["d"]:
                    points[0] = self.detectIntersectionParalelLine(distanсes[3], distanсes[2],
                                                                   points[distanсes[1]["p0"]])
                else:
                    points[1] = self.detectIntersectionParalelLine(distanсes[1], distanсes[2],
                                                                   points[distanсes[3]["p1"]])
        return points
    ...
    async def detectAsync(self, images, outboundWidthOffset=3, outboundHeightOffset=0, fixRectangleAngle=3,
        ...
        """ Main method """
        loop = asyncio.get_event_loop()
        promises = [loop.create_task(self.detectOneAsync(image, outboundWidthOffset=outboundWidthOffset,
                                                         outboundHeightOffset=outboundHeightOffset,
                                                         fixRectangleAngle=fixRectangleAngle, fixGeometry=fixGeometry))
                    for image in images]
        if bool(promises):
            await asyncio.wait(promises)
